...
def dE_da(z, Om):
    a = 1 / (1.0 + z)
    numerator = (3 * (1.0 + 0.0) / (1.0 + z)) * Om * (1.0 + z) ** 3
    denominator = 2 * a**2 * Ez(z, Om)
    return -numerator / denominator


@njit
def DM(z, Om):
    dh_grid = c / Ez(z_grid, Om)
...
for i in range(N):
    ...
    z_i = z_vals[i]
    DM_i = DM(np.array([z_i]), Om_fid_i)[0]
    Ez_i = Ez(z_i, Om_fid_i)
    Ez_DMz_fiducial[i] = Ez_i * DM_i


@njit
def growth_ode(a, y, Om):
    z = 1 / a - 1.0
    E_val = Ez(z, Om)
    dE_da_val = dE_da(z, Om)
...
def chi_squared(theta):
    ...
    q = Ez(z_vals, Om) * DM(z_vals, Om) / Ez_DMz_fiducial

    diff = data["fs8"] - fs8_theory(z_vals, Om, sig8) * shape / q
...
def main():
    ...
    labels = ["$S_8$", "$Ω_m$", "$\sigma_8$", "$g_8$", "$f_{err}$"]
    plot_corner_and_chains(labels, samples, chains_samples)
    plot_predictions(
        fs8_theory=lambda z: fs8_theory(z, Om_50, s8_50),
        data=data,
        q=(Ez(z_vals, Om_50) * DM(z_vals, Om_50) / Ez_DMz_fiducial)
