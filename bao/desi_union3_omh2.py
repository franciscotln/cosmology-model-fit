...
def DM_grid(params):
    """
    returns (DM, DH) grids for interpolation
    """
...
@njit
def bao_theory(z, qty, dm_grid, params):
    DM = DM_z(z, dm_grid)
    DH = DH_z(z, dm_grid)
    ...
    results[DV_mask] = DV_z(z[DV_mask], DM[DV_mask], DH[DV_mask])
...
def get_z_cosmo(params):
    ...
    z_pec = v_km_s / c
    return -1 + (1 + z_cmb) / (1 + z_pec)


def mu_corr(params, dm_grid):
    # for plotting purposes only, not used in likelihood
...
def chi_squared(params):
    ...
    z_cosmo = get_z_cosmo(params)
    delta_sn = mu_vals - mu_theory(params, DM_z(z_cosmo, dm_grid))
    chi_sn = delta_sn @ inv_cov_sn @ delta_sn

    delta_bao = bao_data["value"] - bao_theory(bao_data["z"], desi_qty, dm_grid, params)
...
def main():
    ...
    best_fit = np.array([dM_50, rd_50, H0_50, Omh2_50, v_50])
    MAP_params = samples[np.argmax(log_l)]
    DOF = len(bao_data) + len(z_cmb) - len(best_fit)
    ...
    plot_bao_predictions(
        theory_predictions=lambda z, qty: bao_theory(z, qty, dm_grid_best, best_fit),
        data=bao_data,
    ...
    plot_sn_predictions(
        ...
        y_model=mu_theory(best_fit, DM_z(z_cmb, dm_grid_best)),
...
# v: -3.11 +1.04 -1.06 x 100 km/s
# ΔM: -0.025 +0.025 -0.025 mag
# rd: 146.44 +1.29 -1.28 Mpc
# H0: 69.35 +1.00 -0.99 km/s/Mpc
# Ωm: 0.297 +0.008 -0.008
# ωm: 0.1430 +0.0011 -0.0011
# Chi squared: 32.4 (2.95 sigma significance)
# Log evidence: -29.7 (Δ logZ = 2.5 in favour of velocity step correction)
# Degs of freedom: 30
# ---------------------------------
